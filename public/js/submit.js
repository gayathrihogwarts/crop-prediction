$('.btn').click(function(){
	...
	if(city!="none")
	{
		...
		document.getElementById("search_box2").style.visibility = "hidden";
		document.getElementById("crops").style.visibility = "hidden";
        document.getElementById("back").style.visibility = "visible";
        // document.getElementById("search_categories2").disabled = false;
...
document.getElementsByTagName('script')[0].parentNode.appendChild(script);
	$.post("/location", { location: city,month:month }, (data, status) =>
	 {

			var cropline= data.toString();
       		
       	document.getElementById("search_categories").disabled = true;	
       	document.getElementById("crops").style.visibility = "visible";
       		cropDB = cropline.split(",");
       	// window.alert(cropDB);	
       	var i =0;
		// for (; i < cropDB.length-1; i++)
		if(cropDB.length%2==0)
		{
		while(i < cropDB.length)
		 {
